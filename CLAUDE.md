...
- **`SchedulingSystem.jsx`** — confirm step. **Guest visitors** (not logged in) see a "Seus dados" block collecting name/email/phone/CPF/password; on submit it calls `authService.guestCheckout` which creates the Client and stores the JWT, then the same booking flow runs. **Logged-in clients** with no CPF on file see the legacy inline CPF input. Submit creates the appointment (with optional CPF), then if the response is `pending_payment`, calls `paymentService.createPayment` and does `window.location.href = invoice_url` to send the user to Asaas hosted checkout. Free / B2B bookings keep the old flow — straight to `ConfirmationPage` via location.state. A `pendingAppointmentId` ref guards against double-booking on retry: if payment creation fails after the appointment commits, clicking the button again re-attempts payment only. The therapistId is read from the `?therapistId=N` query param (refresh-safe for campaign links), falling back to router state.
